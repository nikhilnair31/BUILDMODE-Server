...
def relevant(current_user):
    ...
    try:
        ...
        relevant_text_wo_time_text, time_filter = extract_time_filter(relevant_text)
        logger.info(f"relevant_text_wo_time_text: {relevant_text_wo_time_text} - time_filter: {time_filter}")
        
        relevant_text_wo_col_text, color_lab = extract_color_filter(relevant_text_wo_time_text)
        has_color = color_lab is not None
        logger.info(f"relevant_text_wo_col_text: {relevant_text_wo_col_text} - color_lab: {color_lab} - has_color: {has_color}")

        struc_relevant_text = sanitize_tsquery(relevant_text_wo_col_text) if relevant_text_wo_col_text else ""
        logger.info(f"struc_relevant_text: {struc_relevant_text}")

        vec_query = cached_call_vec_api(relevant_text_wo_col_text) if relevant_text_wo_col_text else None
        ...
        params = {
            "userid": userid,
            "fts_query": struc_relevant_text,
            "trgm_query": relevant_text_wo_col_text,
            "vec_query": vec_query,
            "start_ts": time_filter[0] if time_filter else None,
            "end_ts": time_filter[1] if time_filter else None,
            "has_color": has_color,
